...
3. Delete the old `.Codex/` directory
4. Copy the new `.Codex/` in
...
Step 2: PRESERVE USER CUSTOMIZATIONS
  - Save .Codex/references/domain-profile.md if filled in (not just placeholders)
  - Save any custom journal profiles the user added to journal-profiles.md
  - Save .Codex/settings.json (user's permissions and hooks)
  - Save .Codex/settings.local.json if it exists

Step 3: REPLACE
  - Delete old .Codex/ entirely
  - Copy new .Codex/ from the downloaded release
  - Restore saved customizations from Step 2

Step 4: DO NOT TOUCH
  - paper/, scripts/, data/, explorations/, quality_reports/
  - AGENTS.md, Bibliography_base.bib, README.md, .gitignore
...
**No git merge. No upstream remote. No conflicts.** Just delete and replace `.Codex/`.
